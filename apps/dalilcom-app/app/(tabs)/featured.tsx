...
export default function FeaturedTab() {
  ...
  const { state } = useApp();
  const ads = state.ads.filter(ad => ad.category === 'real-estate' && ad.isFeatured);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}><Text style={styles.headerTitle}>الإعلانات المميزة</Text></View>
      <FlatList
        data={ads}
        keyExtractor={item => item.id}
        renderItem={renderItem}
        contentContainerStyle={styles.list}
        ListEmptyComponent={<Empty icon="sparkles" text="لا توجد إعلانات مميزة حالياً" />}
      />
    </View>
  );
}
...
function Empty({ icon, text }: { icon: string; text: string }) {
  return (
    <View style={styles.empty}>
      <NativeIcon name={icon} size={38} color={COLORS.gray400} />

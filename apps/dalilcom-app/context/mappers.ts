...
export function mapAdFromApi(row: any): AdItem {
  ...
  return {
    ...
    images: (row.images || ad.images || []).map((i: any) => typeof i === 'string' ? i : i.image_url).filter(Boolean),
    videos: (row.videos || ad.videos || []).map((v: any) => typeof v === 'string' ? v : v.video_url).filter(Boolean),
